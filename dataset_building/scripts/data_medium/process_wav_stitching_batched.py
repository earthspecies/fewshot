...
def main():
  ...
  parser.add_argument('--info', help = "filepath of csv with input and output info", required=True, type=str)
  parser.add_argument(
  ...
  for i, row in info_df.iterrows():
    ...
    input_blocks_np, input_len_np, sample_rate = _extract_blocks_from_input(
        input_wav_fp, block_size_in_samples, args.input_channels,
        args.scale_input, args.window_type)

    assert sample_rate == args.sample_rate
    
    batch_blocks.append(input_blocks_np)
    input_lens.append(input_len_np)
    batch_chunks.append((batch_position, batch_position+np.shape(input_blocks_np)[0]))
    ...
    if (len(batch_blocks) >= args.batch_size) or (i >= len(info_df)-1):      
      ...
      output_blocks_np = _run_model_for_blocks(
          batch_blocks, args.model_dir, args.checkpoint, args.input_tensor,
          args.output_tensor)
      
      for k, batch_chunk in enumerate(batch_chunks):
        batch_chunk_start, batch_chunk_end = batch_chunk
        _resolve_permutation_and_write_output(
            output_wav_fps[k], sample_rate, output_blocks_np[batch_chunk_start:batch_chunk_end,:,:], input_lens[k],
