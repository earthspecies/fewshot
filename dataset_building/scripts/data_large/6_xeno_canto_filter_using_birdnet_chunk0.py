...
raw_dir = f"/home/davidrobinson/fewshot_data/data_large/{dataset_name}_audio_trimmed"
stems_dir = f"/home/davidrobinson/fewshot_data/data_large/{dataset_name}_audio_mixit"
output_dir = f"/home/davidrobinson/fewshot_data/data_large/{dataset_name}_pseudovox"
data_dir = "/home/davidrobinson/fewshot_data/data_large"
    ...
    for stem_fp in stem_fps:
        ...
        for k, clip in enumerate(clips):
            ...
            try:
                ...
                print(f"Error writing file {target_fp}: {e}")
    return info

raw_audio_fps = sorted(glob(os.path.join(raw_dir, "*.wav")))[0:100]

info_list = []
with ThreadPoolExecutor(max_workers=4) as executor:  # Adjust max_workers as needed
    futures = [executor.submit(process_audio_file, raw_audio_fp) for raw_audio_fp in raw_audio_fps]
    for future in tqdm(as_completed(futures), total=len(futures)):
        info_list.append(future.result())

# Combine all results into a DataFrame and save as CSV
info_df = pd.DataFrame({k: [entry[k] for entry in info_list for k in entry.keys()] for k in info_list[0].keys()})
