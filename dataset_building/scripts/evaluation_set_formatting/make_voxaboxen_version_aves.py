...
datasets = ["katydid", "marmoset", "Spiders", "Anuraset", "carrion_crow", "rana_sierrae", "Powdermill",  "Hawaii", "right_whale", "gibbons", "gunshots", "humpback", "ruffed_grouse"]
...
#         test_manifest.to_csv(test_info_fp, index= False)
        ...
        project_command = f'python main.py project-setup --train-info-fp="{train_info_fp}" --val-info-fp="{test_info_fp}" --test-info-fp="{test_info_fp}" --project-dir="/home/jupyter/fewshot_supervised_aves/{project_name}"\n'
        commands_to_run += project_command
        
        train_command = f'python main.py train-model --project-config-fp="/home/jupyter/fewshot_supervised_aves/{project_name}/project_config.yaml" --lr=.01 --batch-size={batch_size} --name=e2v1 --clip-duration={clip_duration} --segmentation-based --encoder=aves --unfreeze-encoder-epoch=1000000 --n-epochs=100 --rho=1 --delete-short-dur-sec={delete_short_dur_sec} --fill-holes-dur-sec={fill_holes_dur_sec} --exists-strategy=overwrite --patience=10 --omit-empty-clip-prob=0.5\n'
...
with open('/home/jupyter/sound_event_detection/fewshot_supervised_aves.sh', 'w') as f:
