    ...
    for folder in folders:
        ...
        for file in tqdm(files):
            if file[-4:] == '.wav':
                ...
                df = pd.read_csv(folder_path+folder+'/'+annotation)
                for i, row in df.iterrows():
                    if row["Q"] == "POS" and len(events) < shots:
                        events.append(row)
                ...
                to_predict = norm_stft[:, int(np.ceil(float(events[-1]["Endtime"])*sr/hop_len + 1)):]

                result = []
                for event in events:
                    # starttime = float(event[1])
                    starttime = float(event["Starttime"])
                    # endtime = float(event[2])
                    endtime = float(event["Endtime"])
                ...
                for i in range(len(events)):
                    ...
                    for j in range(len(events)):
                        if j != i:
                            ...
                            inner_starttime = float(inner_event["Starttime"])
                            # inner_endtime = float(inner_event[2])
                            inner_endtime = float(inner_event["Endtime"])
                ...
                for i in range(len(result)):
                    ...
                    event_len = int(np.ceil(np.floor(float(events[i]["Endtime"])*sr/hop_len + 1))) - int(np.floor(float(events[i]["Starttime"])*sr/hop_len))
                ...
                for i in range(len(binary_result)):
                    ...
                    starttime = float(events[i]["Starttime"])
                    endtime = float(events[i]["Endtime"])
                    event_stft = norm_stft[2:-2,int(np.floor(starttime*sr/hop_len + 1)):int(np.ceil(endtime*sr/hop_len + 1))]
