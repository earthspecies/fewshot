...
def create_htsat_model(audio_cfg, enable_fusion=False, fusion_type='None', lock = False):
    try:
        ...
        print(f"HTS-AT Model {audio_cfg.model_name} created.")
        print(f"------Audio trainable params: {sum(p.numel() for p in model.parameters() if p.requires_grad)}--------")
...
def load_pretrained_htsat_model(model, pretrained_audio, device):
    ...
    if 'HTSAT' in pretrained_audio:  # official checkpoint
        ...
        keys_to_skip = [
                ...
                "audio_branch.bn0.weight",
                "audio_branch.bn0.bias",
                "audio_branch.bn0.running_mean",
                "audio_branch.bn0.running_var",
                "audio_branch.tscam_conv.weight",
            ]

        # Remove the specified keys
        for key in keys_to_skip:
            if key in audio_ckpt:
